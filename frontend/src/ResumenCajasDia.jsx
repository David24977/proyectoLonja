...
export default function ResumenCajasDia() {
  ...
  const consultarResumen = async (e) => {
    ...
    try {
      const resp = await axios.get(`${API_BASE_URL}/cajas/resumen-dia`, {
        params: { fecha }
      });
  ...
  const total = listado.reduce((acc, item) => acc + item.totalCajas, 0);

  return (
    <div>
      <h2>Resumen diario de cajas devueltas</h2>
      ...
      <table style={{
        ...
        <thead>
          <tr>
            ...
            <th style={{ padding: "0.6em", background: "#e3eafe", color: "#25408f" }}>Cajas</th>
          </tr>
        </thead>
        <tbody>
          {listado.length === 0
            ? (
              <tr>
                <td colSpan={2} style={{ padding: "1.5em", textAlign: "center", color: "#888" }}>
                  No hay devoluciones para ese día.
                </td>
              </tr>
            )
            : listado.map((item, idx) => (
              <tr key={idx}>
                <td style={{ padding: "0.5em" }}>{item.nombreCliente}</td>
                <td style={{ padding: "0.5em" }}>{item.totalCajas}</td>
              </tr>
            ))
          }
        </tbody>
